...
const allTypes = db.prepare('SELECT id FROM appointment_types').all();
const allProviders = db.prepare('SELECT id FROM providers').all();
...
for (const provider of allProviders) {
    for (const type of allTypes) {
        insertLink.run(provider.id, type.id);
    }
}
...
function generateTimeSlots() {
    ...
    const insertMany = db.transaction((providerIds) => {
