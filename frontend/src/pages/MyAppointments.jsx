...
export default function MyAppointments() {
    ...
    useEffect(() => {
        if (member && member.email && !searched && !loading) {
            setSearchType('email')
            setSearchValue(member.email)
            performSearch({ email: member.email })
        }
    ...
    return (
        <div className="page-container animate-fade-in">
            {member ? (
                <div className="mb-6">
                    <h1 className="section-title mb-2 text-primary-900 border-b border-primary-100 pb-3">Member Dashboard</h1>
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
                        {/* Member ID Card */}
                        <div className="lg:col-span-1 bg-gradient-to-br from-primary-700 to-primary-900 rounded-2xl shadow-card text-white p-6 relative overflow-hidden">
                            <div className="absolute top-0 right-0 -mr-8 -mt-8 w-32 h-32 rounded-full bg-white opacity-10"></div>
                            <h2 className="text-sm font-semibold tracking-wider text-primary-100 uppercase mb-4">CareBook Health</h2>
                            <div className="mb-6">
                                <p className="text-2xl font-bold">{member.firstName} {member.lastName}</p>
                                <p className="text-primary-200 text-sm">Member ID: {member.memberId}</p>
                            </div>
                            <div className="space-y-2 mb-4">
                                <div className="flex justify-between">
                                    <span className="text-primary-200 text-sm">Plan</span>
                                    <span className="font-semibold text-sm">{member.planName}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-primary-200 text-sm">Network</span>
                                    <span className="font-semibold text-sm">{member.planNetwork}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-primary-200 text-sm">Group #</span>
                                    <span className="font-semibold text-sm">{member.groupNumber}</span>
                                </div>
                            </div>
                        </div>

                        {/* PCP Details */}
                        <div className="lg:col-span-2 bg-white rounded-2xl shadow-card border border-surface-100 p-6 flex flex-col justify-center">
                            <h3 className="text-lg font-bold text-surface-800 mb-4 border-b border-surface-50 pb-2">Primary Care Provider (PCP)</h3>
                            {member.pcp ? (
                                <div className="flex items-center gap-4">
                                    <div className="w-16 h-16 rounded-full bg-primary-50 flex items-center justify-center text-primary-600 font-bold text-xl shadow-inner border border-primary-100">
                                        {member.pcp.name.split(' ').map(n => n[0]).join('')}
                                    </div>
                                    <div>
                                        <p className="text-lg font-bold text-surface-900">{member.pcp.name}</p>
                                        <p className="text-sm text-surface-500">{member.pcp.clinic}</p>
                                        <p className="text-sm text-surface-500 mt-1">{member.pcp.phone} • Nearest Facility</p>
                                    </div>
                                    <div className="ml-auto">
                                        <Link to={`/providers?q=${encodeURIComponent(member.pcp.name)}`} className="btn-secondary text-sm px-4 py-2">
                                            Book Visit
                                        </Link>
                                    </div>
                                </div>
                                ...
                                <div className="text-surface-500 text-sm">
                                    <p>No Primary Care Provider assigned.</p>
                                    <Link to="/providers?specialty=Family%20Medicine" className="text-primary-600 font-semibold mt-2 inline-block">Find a PCP →</Link>
                                </div>
                            )}
                        </div>
                    </div>
            ...
            {/* Search Form (Only visible for guests) */}
            {!member && (
            ...
            {/* Results */}
            <div className="mt-8">
                {member && <h3 className="text-xl font-bold text-surface-900 mb-4 border-b border-surface-100 pb-2">Your Appointments</h3>}

                {loading ? (
                    <LoadingSpinner text="Searching appointments..." />
                ) : error ? (
                    <ErrorMessage message={error} onRetry={member ? () => performSearch({ email: member.email }) : handleSearch} />
                ) : searched && appointments && appointments.length === 0 ? (
                    <div className="text-center py-16 bg-white rounded-2xl shadow-sm border border-surface-100 p-8">
                        ...
                        <p className="text-surface-500 text-sm mb-6">We couldn't find any appointments matching your records.</p>
